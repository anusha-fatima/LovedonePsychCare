...
import { ArrowRight, CheckCircle, Mic, Heart, Shield, Calendar, MessageCircle, User, Mail, FileText } from 'lucide-react';
...
export default function BookingPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Store user data in localStorage to use on dashboard
          ...
          {step === 'terms' && (
            ...
            >
              ...
              <div className="flex justify-center mb-6">
                <div className="p-4 rounded-full" style={{ background: colors.iconBg }}>
                  <Mic className="w-6 h-6" style={{ color: colors.textPrimary }} />
                </div>
              </div>

              {/* Heading */}
              <h2 className="text-3xl md:text-4xl font-light mb-4 transition-colors duration-300" style={{ color: colors.textPrimary }}>
                Before We Begin
              </h2>

              <p className="text-sm md:text-base mb-8 max-w-xl mx-auto transition-colors duration-300" style={{ color: colors.textTertiary }}>
              ...
              <div className="text-left space-y-4 text-sm mb-8">
                {[
                  ...
                  <div key={i} className="flex gap-3">
                    <CheckCircle className="w-4 h-4 mt-1 shrink-0" style={{ color: colors.textSecondary }} />
                    <p className="transition-colors duration-300" style={{ color: colors.textSecondary }}>{term}</p>
                  </div>
                ))}
              </div>
          ...
          {step === 'form' && (
            ...
            >
              <div className="flex justify-center mb-4">
                <div className="p-3 rounded-full" style={{ background: colors.iconBg }}>
                  <Heart className="w-6 h-6" style={{ color: colors.textPrimary }} />
                </div>
              </div>

              <h2 className="text-3xl font-light text-center mb-2 transition-colors duration-300" style={{ color: colors.textPrimary }}>
                Tell Us About You
              </h2>

              <p className="text-center text-sm mb-8 transition-colors duration-300" style={{ color: colors.textTertiary }}>
                This helps us connect you with the right support 💙
              </p>

              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="relative">
                  <User className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5" style={{ color: colors.textTertiary }} />
                  <input
                    type="text"
                ...
                <div className="relative">
                  <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5" style={{ color: colors.textTertiary }} />
                  <input
                    type="email"
                ...
                <div className="relative">
                  <FileText className="absolute left-4 top-4 w-5 h-5" style={{ color: colors.textTertiary }} />
                  <textarea
